    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-[300px]">
          <DialogHeader>
            <DialogTitle className="text-base">Eliminar usuario</DialogTitle>
          </DialogHeader>
          <div className="py-2">
            <p className="text-sm text-muted-foreground">
              ¿Estás seguro de que deseas eliminar el usuario con ID{" "}
...
export default function PuntosPage() {
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      setLoadingRows(true);
      setError(null);
      const response = await fetch("/api/dhl_users", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const columns = useMemo<ColumnDef<RowData>[]>(
    () => [
      { id: "NOMBRE_COMPLETO", header: "Nombre Completo", priority: 1 },
      { id: "ROL", header: "Rol", priority: 2 },
