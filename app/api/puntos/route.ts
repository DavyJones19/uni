...
const PUNTOS_URL = process.env.EXTERNAL_PUNTOS_URL || "";
// EXTERNAL_PUNTOS_METHOD permite forzar GET si la API externa no usa POST.
const PUNTOS_METHOD =
  (process.env.EXTERNAL_PUNTOS_METHOD || "POST").toUpperCase() === "GET"
    ? "GET"
    : "POST";
...
export async function POST(request: NextRequest) {
  if (!PUNTOS_URL) {
    return NextResponse.json(
      { error: "Falta configurar EXTERNAL_PUNTOS_URL" },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Error al conectar con la API externa de puntos" },
      { status: 500 },
    );
  }
}
