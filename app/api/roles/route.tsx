...
const ROLES_URL = process.env.EXTERNAL_ROLES_URL || "";
// EXTERNAL_ROLES_METHOD permite forzar GET si la API externa no usa POST.
const ROLES_METHOD =
  (process.env.EXTERNAL_ROLES_METHOD || "POST").toUpperCase() === "GET"
    ? "GET"
    : "POST";
...
export async function POST(request: NextRequest) {
  if (!ROLES_URL) {
    return NextResponse.json(
      { error: "Falta configurar EXTERNAL_ROLES_URL" },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Error al conectar con la API externa de roles" },
      { status: 500 },
    );
  }
}
